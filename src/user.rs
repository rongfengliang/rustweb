pub mod modle {
    ...
    impl User {
        ...
        pub fn mysqldata(&self) {
            let pool = my::Pool::new("mysql://root:dalongrong@127.0.0.1:3307").unwrap();
            ...
            let selected_payments: Vec<Payment> = pool.prep_exec(
                "SELECT customer_id, amount, account_name from tmp.payment",
                (),
            ).map(|result| {
